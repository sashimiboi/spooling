...
from spooling.ingest import _store_session, _embed_session, _mark_synced
...
def watch(embed: bool = True):
    ...
    # Fallback: if no providers connected, at least watch session files
    if not watch_targets:
        from spooling.providers.session_file import SessionFileProvider
        fp = SessionFileProvider()
        fp_path = fp.resolved_data_path()
        if fp_path.exists():
            watch_targets.append((fp, fp_path))
