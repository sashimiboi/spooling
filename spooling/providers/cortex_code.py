...
extract text, tool calls, and tool results the same way, but the wrapping is flat (`content` at top level, no nested `message`
object) so we can't reuse `parse_session_file` from spooling.parser directly.
"""
...
from spooling.config import CORTEX_CONVERSATIONS_DIR
