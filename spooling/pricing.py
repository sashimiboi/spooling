...
Spooling's formula charges these four components separately, which is more
...
CACHE_DIR = Path(os.getenv("SPOOLING_CACHE_DIR", str(Path.home() / ".spool")))
...
class ModelRates:
    ...
    table encodes what providers actually bill.
    """
...
def _fetch_remote() -> dict:
    req = urllib.request.Request(
        PRICING_URL,
        headers={"User-Agent": "spooling-pricing/1"},
    )
...
def _candidate_keys(model: str) -> list[str]:
    """Candidate LiteLLM keys to try for a given Spooling model name.

    LiteLLM sometimes keys entries as ``gpt-4o`` and sometimes
    prefixes them with a provider like ``openai/gpt-4o``.
    Spooling's ingests store the raw model slug reported by the SDK, which
    ...
    # same as the global model), and recurse the extracted stem.
...
def get_rates(model: str | None, provider_id: str | None = None) -> ModelRates:
    ...
    # Fallback — use the hard-coded DEFAULT_PRICING in $/Mtok.
